    ...
    return (
      <div style={overlay} onClick={onClose}>
        <div style={modal} onClick={e => e.stopPropagation()}>
          <div style={mh}>
            <div>
              <div style={{ fontSize: 15, fontWeight: 300, color: '#fff', letterSpacing: -0.3 }}>Verify Your Email</div>
          ...
          <div style={mb}>
            ...
            {resendMsg && <div className="alert alert-success" style={{ marginBottom: 13 }}>{resendMsg}</div>}

            <div style={{ fontSize: 13, color: '#3E3A34', marginBottom: 16, lineHeight: 1.6 }}>
              We sent a 6-digit verification code to <strong>{form.email}</strong>. Enter it below to activate your account.
            </div>
            ...
            <button className="btn btn-gold" style={{ width: '100%', justifyContent: 'center', marginTop: 4, borderRadius: 4 }} onClick={submitCode} disabled={loading}>
              {loading ? 'Verifying…' : 'Verify & Continue →'}
            </button>

            <div style={{ textAlign: 'center', marginTop: 12, fontSize: 11, color: '#9A9488' }}>
              Didn't get it? <button style={{ background: 'none', border: 'none', color: '#A88C38', cursor: 'pointer', fontSize: 11 }} onClick={resend}>Resend code</button>
            </div>
          </div>
        </div>
      </div>
          ...
          {mode === 'register' && (
            <>
              ...
              <div className="form-group">
                ...
                <div style={{ fontSize: 9.5, color: '#9A9488', marginTop: 4 }}>A verification code will be sent · Required to access full reports</div>
              </div>

              <div className="form-group">
                <label className="form-label">Phone Number</label>
                <input className="input" type="tel" value={form.phone} onChange={set('phone')} placeholder="+1 (305) 000-0000" />
                <div style={{ fontSize: 9.5, color: '#9A9488', marginTop: 4 }}>Optional</div>
              </div>
            </>
          )}
